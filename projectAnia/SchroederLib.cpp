...
#define EXPORTING_SchroederLib 1
#include "SchroederLib.h"
    ...
    mclOutputHandlerFcn print_handler)
{
        ...
        return false;
    {
        mclCtfStream ctfStream = 
            mclGetEmbeddedCtfStream((void *)(SchroederLibInitializeWithHandlers));
        if (ctfStream) {
...
{
    return SchroederLibInitializeWithHandlers(mclDefaultErrorHandler, 
...
LIB_SchroederLib_CPP_API 
void MW_CALL_CONV SchroederAlgorithm(int nargout, mwArray& schroederAudio, const mwArray& 
                                     fileLocation, const mwArray& reverbAmount)
{
    mclcppMlfFeval(_mcr_inst, "SchroederAlgorithm", nargout, 1, 2, &schroederAudio, &fileLocation, &reverbAmount);
}
